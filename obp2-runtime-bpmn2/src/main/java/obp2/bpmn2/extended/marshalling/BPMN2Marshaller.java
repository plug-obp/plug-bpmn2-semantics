...
public class BPMN2Marshaller implements IMarshaller<BPMN2RuntimeConfiguration, BPMN2FlowAction, Void> {

    private BPMN2Module module;

    @Override
    public ILanguageModule<BPMN2RuntimeConfiguration, BPMN2FlowAction, Void> getModule() {
        return null;
    }

    @Override
    public void setModule(ILanguageModule<BPMN2RuntimeConfiguration, BPMN2FlowAction, Void> module) {
    ...
    @Override
    public byte[] serializeConfiguration(BPMN2RuntimeConfiguration configuration) {
    ...
    @Override
    public BPMN2RuntimeConfiguration deserializeConfiguration(byte[] buffer) {
    ...
    static private void writeState(BPMN2RuntimeConfiguration state, OutputStream os) throws IOException {
        Marshaller.writeIntArray(state.tokens, os);
    }

    static private BPMN2RuntimeConfiguration readState(InputStream is) throws IOException {
        int[] tokens = new int[Unmarshaller.readInt(is)];
        for (int index = 0; index < tokens.length; index++) {
            tokens[index] = Unmarshaller.readInt(is);
        }
        return new BPMN2RuntimeConfiguration(tokens);
    }
